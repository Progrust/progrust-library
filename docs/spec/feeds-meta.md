...
- **R-7**: canonical URLは `https://blog.progrust.com` ドメインで出力する。
...
- Cloudflare Web Analyticsのトークンは環境変数 `PUBLIC_CF_BEACON_TOKEN` で注入し、設定時のみbeaconタグを出力する条件レンダリングで確定（T6-2）。ビルドはGitHub Actions上で行うため、実トークンは **GitHub Actions Secrets**（`PUBLIC_CF_BEACON_TOKEN`）に登録しビルドステップの環境変数として注入する（T6-4。[deploy.md](deploy.md) 参照）。
