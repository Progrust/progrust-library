...
- **R-13**: 左に目次を表示する（xl（1280px）未満ではフローティングボタン押下でポップアップ）。対象は**h2〜h4**（本文先頭のh1はページタイトルと重複するため目次に含めない。脚注ラベルも除外する）。スクロールに応じて現在地の見出しをハイライトする。本の章では、章目次（本の全章リスト）も同時に表示する（本文の目次は開いているページのみ）。
