...
- **R-3**: フッター: コピーライト表記 + GitHub / X / RSS へのテキストリンク。
...
- **R-7**: ヒーローにはSNS/GitHubアイコンのみ表示する（プロフィールは置かない）。
- **R-26**: ページ最下部に「このサイトについて」（サイト紹介文 + 辞書・記事・本の3形式紹介カード）を表示する。内容は `src/pages/index.astro` に直書きする。（旧R-25。R-25がコード記法除去の要件と重複採番されていたため、参照の少ない本要件をR-26へ振替）
...
- **R-12**: 中央に タイトル / タグ一覧 / 更新日 / 本文 を表示する。タグクリックでタグ詳細ページへ遷移する。ただし**章はタグを表示しない**（章ヘッダーは本タイトルの補助行と本文に絞る。確定モック `book-chapter.html`（モック本体はリポジトリ外・claude design 上。所在は [`ui-design-spec.md`](../ui-design/ui-design-spec.md) 冒頭）/ 同文書 §312 に従う）。章のタグは frontmatter に保持し、タグ検索・タグ詳細（R-19）の対象には含める。
- **R-13**: 左に目次を表示する（xl（1280px）未満、および辞書ペインの拡大表示中（[`wikilink-ui.md`](wikilink-ui.md) R-23）ではフローティングボタン押下でポップアップ）。対象は**h2〜h4**（本文先頭のh1はページタイトルと重複するため目次に含めない。脚注ラベルも除外する）。スクロールに応じて現在地の見出しをハイライトする。本の章では、章目次（本の全章リスト）も同時に表示する（本文の目次は開いているページのみ）。目次（章詳細の複合目次・本トップの章目次サイドバー（R-17）を含む）が画面の縦幅を超える場合は、sticky固定のまま目次カラム内で縦スクロールできる（上限の具体値は [`ui-design-spec.md`](../ui-design/ui-design-spec.md)「レイアウト」）。
...
（複数のコードブロックを1つのプロジェクトとしてPlaygroundで開く記法 `:::project` は [`playground-project.md`](playground-project.md) を参照。R-23の複数ファイル版で、要求仕様・受入基準とも同文書が持つ）
...
（R-20 はプロフィールページの要件だったが、プロフィールページ廃止に伴い削除。欠番とし番号は再利用しない）
...
- **AC-7**: 詳細ページの目次にh2〜h4の見出しが階層表示され、クリックで該当見出しへ移動する。見出しが多く画面の縦幅を超える場合も、目次カラム内のスクロールで末尾の項目に到達できる。（R-13）※スクロールは目視確認
