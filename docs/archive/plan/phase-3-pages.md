...
本トップ（`/books/[slug]`）・章詳細（`/books/[book]/[chapter]`）を初めて静的表示できる状態にし、章目次サイドバー・複合目次・前後章ナビを実装した。既存の公開フィルタ（`getPublicBooks`/`getPublicChapters`）・`sortChapters`・`tocHeadings` を再利用し、AC の中核ロジックは純関数として `content.ts` に追加した。

**実装内容（変更・新規ファイル）**

- `src/lib/content.ts`: 純ロジックを追加。`chapterOrderLabel()`（連番のゼロ埋め2桁ラベル）・`chapterNav()`（連番昇順リストで現在章の前後の**公開章**を返す。間の非公開章はスキップ。R-16/AC-4 の中核）・`chapterLedger()`（章目次の台帳データ＝slug/連番/URL/公開状態。URL規則 content-model R-8 を集約）と型 `ChapterLedgerItem`。
- `src/components/ChapterToc.astro`（新規）: 章目次／複合目次。`// chapters` eyebrow + 本タイトルリンク + 台帳リスト。`currentSlug` 指定で現在章を強調（`aria-current`・リンクなし）し本文見出しを `ml-[1.75rem] border-l` でネスト（`tocHeadings` 絞込・`data-toc-link` 付与で `toc.ts` の現在地追従に連動）。`label` 指定で本トップのモバイル章目次セクション（見出し+`N chapters` 件数）にも流用。
- `src/components/ChapterNav.astro`（新規）: 前後章ナビ（`grid sm:grid-cols-2` のカード。`← prev / NN` / `next / NN →`。先頭/最終は空セル。R-16/§313）。
- `src/components/MobileNav.astro`（後方互換の小改修）: 既定 slot を追加。渡されればシート本体を差し替える（章詳細は複合目次を差込・§314）。未指定なら従来どおり `Toc` フォールバックで辞書・記事は無改修動作。
- `src/layouts/ChapterLayout.astro`（新規）: 章詳細の3カラム（`DetailLayout` 踏襲・左=220px 複合目次 / 中央=章ヘッダー+本文+`ChapterNav` / 右=`DictPane` 枠）。ヘッダーは eyebrow `// book chapter` → mono補助行 `NN / 本タイトル`（本トップへのリンク）→ h1 → `created / updated`（タグ・画像なし。§312）。`MobileNav` に複合 `ChapterToc` を差し込み、`toc.ts` を `initToc()`。
- `src/pages/books/[slug].astro`（新規・本トップ R-17/AC-5）: `getPublicBooks` で列挙。本 slug 配下の章を `sortChapters`→`chapterLedger` 化（**本番は公開章のみ=AC-5**）。`max-w-5xl` 2カラム（左=`ChapterToc` sticky / 右=ヒーロー[書影 `aspect-[600/850]`・タイトル/概要/タグ/日付]+`render(book)` 本文）。モバイルは本文後に `ChapterToc label="目次"`。
- `src/pages/books/[book]/[chapter].astro`（新規・章詳細 R-12/R-13/R-16/AC-4）: `getPublicChapters` で列挙（本番は公開章のみ生成・非公開は404）。本ごとに `chapterLedger`・`chapterNav` を算出し、前後章を表示データ（href/order/title）へ変換して `ChapterLayout` へ渡す。
- `tests/lib/content.test.ts`: `chapterNav` の `[AC-4]`（中間/非公開スキップ/先頭・最終 null/現在章不在）、`chapterOrderLabel`・`chapterLedger` のテストを追加（83件へ）。
- `docs/architecture.md` §6: `ChapterLayout`・`ChapterToc` を追記し、`DetailLayout` の共用範囲（辞書・記事）と本トップ2カラムの別扱いを明記。

**満たした受入基準**: pages.md **AC-4**（前後章ナビが連番順・間の非公開章スキップ・先頭/最終の欠落）/ **AC-5**（本トップ章目次が本番で公開章のみ）。AC-1 の当該分（`/books/[slug]`・`/books/[book]/[chapter]` 生成）。R-13 の章拡張（複合目次）・R-17 を実装。

**特記事項（申し送り）**

- **右ペイン動作・使用辞書一覧・逆リンクは P4**（wikilink-ui）。`DictPane` は静的枠のまま。
- **dev の非公開識別バッジは未実装**（デザイン未確定・content-model §5）。ナビは R-16 通り非公開をスキップ、サイドバー台帳は dev では全章表示（バッジは後続）。
- 本トップの本文（`render(index.md)`）はダミーが本文先頭に h1 を持つため、ヒーローのタイトルと重複表示される（T3-4 と同じ検証データ由来。実コンテンツでは非該当）。
- 本トップは2カラム・章詳細は3カラムでヘッダーも大きく異なるため、`DetailLayout` を共用せず `ChapterLayout`（新規）＋`BaseLayout` 直利用とした（[architecture.md §6](../../architecture.md) に反映）。

**テスト方針**: architecture.md §10 に従い、ページ/UI は自動テスト対象外（build 成功 + 目視/生成物確認）。前後章ナビ・台帳・連番ラベルは純データロジックのため vitest 対象（`[AC-4]` 含む）。

**レビュー対応（[T3-5レビュー](../../archive/review/T3-5.md) R-1/M-1/M-2 反映）**: ①pages.md R-12 に**章はタグ非表示**の例外を明記（確定モック §312 を SSoT へ還流。R-1）。②`[chapter].astro` の `toTarget` を台帳項目の `title` 参照に統一し `entry.data.title` との二重参照を解消（M-1）。③`chapterNav` の非公開スキップテストに next 方向のケースを追加（M-2。84件へ）。M-3（連番欠落時の `"00"` 表示）は命名規則違反のビルド時検知として今回スコープ外。レビュー総合判定は**承認（要修正なし）**。

**検証結果**:

- `npm run check`（format:check / lint / typecheck / vitest 83 tests）: PASS
- `npx astro build`: 成功（37ページ）。生成物で確認:
  - AC-4: `/books/rust-web-app-book/{intro,environment,first-api}` 生成。intro=`next / 02` のみ・environment=`← prev / 01`+`next / 03`・first-api=`← prev / 02` のみ（先頭/中間/最終のナビが連番順）。非公開スキップは `chapterNav` の `[AC-4]` テストで担保。
  - AC-5: 本トップ `/books/rust-web-app-book` の章目次は公開3章のみ。
  - 複合目次: 章ページに `aria-current="page"`（desktop+mobile の2箇所）・本文見出しの `data-toc-link` ネストを確認。
  - 本トップモバイル章目次セクション `3 chapters` を確認。
...
- （本コミット）docs: T3-5の実施履歴とコンポーネント構成を追記

### T3-6

タグ一覧・タグ詳細・プロフィール・404 を実装し、P3 の全画面（[pages.md R-1](../../spec/pages.md) の全 URL）を静的表示できる状態にした。既存の公開ゲッター・`tagCounts`・`KindBadge`・台帳行/ページヘッダーのマークアップパターンを再利用し、新規コンポーネントは作らず各ページを 1 カラムの直書きで構成した。

**実装内容（変更・新規ファイル）**

- `src/lib/content.ts`: タグ詳細の純ロジックを追加。型 `TagLedgerItem`（種別横断の台帳1行）・`tagLedger()`（指定タグを持つ項目を `created_at` 降順・同日 title 昇順で返す。章 kind:"book" も対象＝AC-6 の中核）。タグ集合の列挙・件数は既存 `tagCounts()` を流用。
- `src/pages/tags/index.astro`（新規・R-18）: 辞書・記事・本・章を合算し `tagCounts` で全公開タグを件数集計（トップの `tagCount` と同母集団=52件）。件数降順チップクラウド（各チップは `/tags/[encodeURIComponent(tag)]` へのページ遷移リンク。絞込 UI の `ListFilter` チップとは別物）。
- `src/pages/tags/[tag].astro`（新規・R-19/AC-6）: `getStaticPaths` で全タグを列挙し、種別横断の `TagLedgerItem[]` を組み立てて `tagLedger` で並べ props 経由で渡す。章は `本タイトル › 章タイトル` 併記・バッジ「本」・本タイトルは `getPublicBooks` の id→title Map で解決。台帳リストはトップ新着と同一グリッド、右端は当該タグを除いた他タグ2件（`hidden sm:flex`）。戻るボタン「← タグ一覧」を先頭に配置。
- `src/pages/profile.astro`（新規・R-20）: 確定モック `profile.html` を忠実転記（自己紹介はモックのダミーテキストのまま）。`BaseLayout` 内に `// profile`（アバター枠 + SNS ラベルボタン `SITE_LINKS`）と `// about this site`（3形式カード。バッジは `KindBadge` 再利用）の 2 セクション。
- `src/pages/404.astro`（新規・R-21/AC-8）: 専用モック無しのためデザインシステムから構成。`BaseLayout`（ヘッダー/フッター=共通デザイン）+ `// 404` eyebrow・大見出し・説明・トップへ戻るボタン。
- `tests/lib/content.test.ts`: `tagLedger` の vitest を追加（`[AC-6]` 章の混入・降順/タイブレーク・非該当除外・元 tags 保持。89件へ）。

**満たした受入基準**: pages.md **AC-1**（`/tags/`・`/tags/[タグ名]`・`/profile/`・`/404.html` 生成）/ **AC-6**（タグ詳細に章コンテンツが「本」バッジ付きで表示）/ **AC-8**（共通デザインの 404）。R-18〜R-21 を実装。

**特記事項（申し送り）**

- **日本語タグの percent-encoding**（T3-4 M-4）: `getStaticPaths` に生タグ名を渡すと Astro が percent-encode したディレクトリ（例 `/tags/Web開発/index.html`）を生成し、リンク側の `encodeURIComponent(tag)`（`DetailLayout`/`books/[slug]`/タグ一覧）と一致することをビルドで確認。整合済み。
- タグの母集団は辞書・記事・本・章の全公開コンテンツ（R-18。トップの `tagCount` と一致）。確定モックの「51 tags」は架空データで、実ダミーコンテンツでは 52 件。
- **dev の非公開識別バッジは未実装**（デザイン未確定・content-model §5）。タグ集計・タグ詳細は公開ゲッター（本番は公開のみ）に依存し、本番セマンティクスで R-18/R-19 を満たす。

**テスト方針**: architecture.md §10 に従い、ページ/UI は自動テスト対象外（build 成功 + 目視/生成物確認）。タグ詳細の絞込＋並びは純データロジックのため `tagLedger` を vitest 対象とした（`[AC-6]` 含む）。

**検証結果**:

- `npm run check`（format:check / lint / typecheck / vitest 89 tests）: PASS
- `npx astro build`: 成功（92ページ）。生成物で確認:
  - AC-1: `/tags/index.html`・`/profile/index.html`・`/404.html`・各 `/tags/[タグ]/index.html`（日本語タグは percent-encode 済みディレクトリで生成）。
  - AC-6: `/tags/Web開発` に「本」バッジの章行 `Rustで始めるWeb API開発 - 実践ガイド › イントロダクション` が `created_at` 降順で表示。
  - AC-8: `/404.html` にヘッダー/フッター + 「トップへ戻る」（`href="/"`）。
  - タグ一覧の件数（52）がトップの TAGS 件数と一致・チップ 52 個。
