...
- 不採用の比較案（参考として残置）: `top-a-warm.html` / `top-c-balance.html` / `top-b2-sidebar.html` / `top-b3-catalog.html`
...
| 画面 | コンテナ | 構成 |
| --- | --- | --- |
| トップ / 辞書一覧 / プロフィール | `max-w-6xl px-4 sm:px-8` | 1カラム |
| 辞書詳細（記事・章も同様の想定） | `max-w-7xl px-4 sm:px-8` | `lg:grid lg:grid-cols-[200px_minmax(0,1fr)_320px] lg:gap-10`（左=目次 / 中央=本文 / 右=使用辞書+サイドペイン） |
...
`font-mono text-[11px] px-2.5 py-1 rounded border` + 出現件数。選択時は `border-accent dark:border-naccent text-accent dark:text-naccent` をclassList.toggleで付与し `aria-pressed` を更新（トークン定義順でaccentがline/subより後のため上書きが効く）。タグはAND、キーワードはタイトル+タグの部分一致。件数表示は `N / 総数`、0件時はメッセージ表示。タグが多い場合は上位12個 + 「+ N tags」で展開。
...
### `:::message` の変換後スタイル（★message-variants.htmlで確定）
