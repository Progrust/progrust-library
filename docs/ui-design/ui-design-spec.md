...
- 確定モック
  ...
  - 辞書ペイン拡大表示: `dict-modal-compare.html`（案D: 非モーダル右ドロワー+デュアルカラムで確定。不採用の案A/B（中央モーダル標準幅/ワイド）・案C（モーダル型スライドオーバー）も比較用に残置）
...
| アクセント・インライン強調文字 | `accent` #C63D22 | `naccent` #D4715A |
  ...
  theme: {
    extend: {
      colors: {
        ...
        nstrong: '#F1EAE1', nink: '#E4DCD1', nsub: '#9C9186', naccent: '#D4715A'
      },
      fontFamily: {
...
- **スクロールバー**: 全域で細幅（WebKit 6px / Firefox `thin`）・トラック透明。サムは `line`（hover で `sub`）・角丸は幅の半分。コードブロックはダーク面のためサムをライト `#4a4238` / ダーク `line` に切替（枠線色と同じペア。hover は共通どおり `sub` で明るくなる）。辞書サイドペイン・ホバープレビュー・左右サイドレール（`[data-side-rail]`）は表示幅が狭いため WebKit 4px に上書き。左右サイドレールのスクロールバーはさらに**レールにホバーしている間のみ表示**（`overflow-y` を非ホバー時 `hidden` ⇔ ホバー時 `auto` で切替。WebKit カスタムスクロールバーは祖先の `:hover` によるサム色切替が再描画されない既知バグがあるため、色ではなく overflow 自体を切り替える。`scrollbar-gutter: stable` でガター幅を常時確保しレイアウトシフトを防ぐ。スクロール位置は非表示中も保持される。ペイン内部・プレビューは常時表示のまま）
...
- 辞書ペインのドロワー表示中（lg以上・`html[data-dict-drawer-open]`）は右カラムと目次カラムを `display: none` にし、グリッド列をクライアント側CSSで本文のみに切り替え、グリッドコンテナの `margin-right` でドロワー幅+gapぶん本文を左へ寄せる。目次はモバイル同様のフローティングボタン+ボトムシートに切り替える（詳細は「辞書ペインの拡大表示」）
...
- キーワード（+ HTMLタグ名）: `#D4715A`（2026-07にUIアクセント`naccent`と同時に#F0684Aから彩度を落とした）
...
| 追加行 マーカー `+` | `#9BB86E` |
| 削除行 背景 | `rgba(212, 113, 90, 0.13)`（シンタックスのキーワード赤 #D4715A ベース） |
| 削除行 マーカー `-` | `#D4715A` |
...
.line.diff.remove { background: rgba(212, 113, 90, 0.13); }
.line.diff.remove::before { content: '-'; color: #D4715A; }
...
```html
<code class="font-mono text-[13px] px-1.5 py-0.5 mx-[0.15em] whitespace-pre bg-[#EBE7DF] dark:bg-[#171411] text-accent dark:text-naccent rounded border border-line/70 dark:border-nline/70">&mut</code>
```

> [!info] 検討の経緯
> ダーク面（E案準拠）→ 文中で主張が強い。文字を旧naccent #F0684A 固定 → ライトでコントラスト不足（約3:1）。最終的に bg=旧A案の #EBE7DF、文字=`text-accent dark:text-naccent`（ライト #C63D22 / ダーク #D4715A）で確定。
...
html.dark .dict-link { color: #D4715A; }
```
...
### 辞書ペインの拡大表示（右ドロワー・★dict-modal-compare.html 案Dで確定）
...
- **拡大中の目次**: モバイル・中間幅と同じフローティング「目次」ボタン+ボトムシートに切り替える。ボタンコンテナ（`data-mobile-nav`）と目次シート（`data-toc-sheet`）は通常 `xl:hidden` だが、ドロワー表示中は xl 以上でも表示する（辞書ボタンは `lg:hidden` のため出ない）。目次ボタン（z-30）はドロワー（z-20）の上に重なる
- **開閉トランジション**: opacity + `translateX(1.5rem)`・180ms・`motion-reduce` で無効化。閉状態は `display: none` ではなく `visibility: hidden` 系で保持し、閉じていてもスクロール量を測定できるようにする
- **スクロール同期**: ペイン本文⇔ドロワー本文で `scrollTop / (scrollHeight - clientHeight)` の比率換算。順序は「開く: ペイン読取→右カラム非表示→ドロワーへ設定」「閉じる: ドロワー読取→右カラム再表示→ペインへ設定」（`display: none` 中はスクロール量が取れないため厳守）

クライアントJSが参照する `data-*` 属性コントラクト（SSoT）:
...
| `data-mobile-nav` | フローティングボタンのコンテナ | ドロワー表示中に xl 以上でも表示 |
| `html[data-dict-drawer-open]` | ルート要素 | 開状態フラグ（CSS切替の起点） |
...
| danger | `accent` #C63D22 | `naccent` #D4715A | 八角形！ |
...
16. 右レールのスクロールは15の「レール全体1コンテナ」から**使用辞書一覧のみスクロール**へ変更（2026-08）。レール全体方式はペイン本文の内部スクロールと入れ子になり操作が重なるため、ペインは位置固定とし、超過分は一覧ラッパの `min-h-0 overflow-y-auto` が受ける flex 構成にした。左（目次カラム）は単一スクロールで入れ子が生じないため、カラム全体スクロールのまま
17. 辞書ペインの拡大表示（GitHub issue #1）は4案比較（`dict-modal-compare.html`: 案A中央モーダル標準幅42rem / 案B中央モーダルワイド56rem / 案Cモーダル型右スライドオーバー / 案D非モーダル右ドロワー+デュアルカラム）→ **案Dを採用**（2026-08）。issue原要件の「拡大表示中は元の画面の操作は不可」は、本文と辞書を同時に参照・操作できるデュアルカラムの価値を優先して非モーダルへ変更（issue #1コメントで記録）。これに伴い枠外クリックでは閉じない（本文側クリックは通常操作のため）。拡大中は右レールを非表示にして目次+本文+ドロワーの構成とし、影なし・`border-l` のみで「オーバーレイではなくレイアウトの一部」に見せる
18. 拡大表示中の目次は17の「目次カラム維持」から**モバイル同様のフローティングボタン+ボトムシートへの切替**に変更（2026-08）。押し出しで狭くなった本文の横幅を目次カラムぶん取り戻し、拡大中の構成を「本文+ドロワー」の純粋な2カラムに揃える。目次への到達手段はモバイルと同じ文法で維持する
