...
- 辞書ペインのドロワー表示中（lg以上・`html[data-dict-drawer-open]`）は右カラムを `display: none` にし、グリッド列をクライアント側CSSで目次+本文に切り替え、グリッドコンテナの `margin-right` でドロワー幅+gapぶん本文を左へ寄せる（詳細は「辞書ペインの拡大表示」）
...
- **レイアウト切替**: 開いている間は右カラム（ペイン+使用辞書一覧）を `display: none` にし、グリッドを目次+本文列に切替。グリッドコンテナに `margin-right: max(0px, calc(var(--dict-drawer-w) + 2.5rem - max(0px, (100vw - 96rem) / 2)))` を与えて本文をドロワー幅+gap 2.5remぶん左へ寄せる（減算項は中央寄せコンテナの余白ぶんの補正。超ワイド画面で本文が過剰に左へ寄るのを防ぐ）
...
| `data-detail-grid`（+ 目次列があれば `data-has-toc`） | 詳細ページのグリッドコンテナ | グリッド列の切替 |
