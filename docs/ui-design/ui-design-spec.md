...
| 辞書詳細（記事も同様の想定） | `max-w-[96rem] px-4 sm:px-8` | `lg:grid lg:grid-cols-[240px_minmax(0,1fr)_384px] lg:gap-10`（左=目次 / 中央=本文 / 右=サイドペイン+使用辞書）。目次対象見出し（h2〜h4）が無いページでは左カラムを省略し `lg:grid-cols-[minmax(0,1fr)_384px]` の2カラムにする（本文が左の空きを使う） |
| 章詳細 | `max-w-[96rem] px-4 sm:px-8` | 辞書詳細と同じ3カラム。ただし左カラムは `264px` で**複合目次**（章リスト+現在章の本文見出し）に差し替え |
...
- ホバー: プレビュー小窓（`w-96 max-h-72 overflow-y-auto rounded border bg-card shadow-xl p-4`、fixed配置。幅は右カラム=384pxに揃える）に辞書の内容（後述の「ペイン/プレビューの内容表示」の compact 形）を表示。リンク下に出し、画面下端にかかる場合は上に反転。プレビュー内の辞書リンクはさらにプレビューしない
