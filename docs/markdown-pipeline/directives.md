...
2. **変換**: `containerDirective` visitorで、同じchildrenを持つ`blockquote`ノード（`data.hName`/`hProperties`で実際のHTML要素に相殺）へ`ctx.replaceNode`する。**mdastプラグインのみで完結**（hastプラグイン不要）。`setProperty(node, 'data', { hName })`だけのremark-directive方式はノード型が`containerDirective`のまま残り、内部の脚注が変換されない（落とし穴6）ため採用しない
...
> **注意**: 以下の雛形は技術検証時点のsetProperty方式。本番実装（`plugins/directives.mjs`）は落とし穴6（directive内の脚注）対応のため、nodeへのsetPropertyを`blockquote`+`hName`への`replaceNode`に置き換えている。label等**子ノード**へのsetProperty・figureのclone後置はそのまま有効。
...
5. **既存ノードの「移動」はremove+insertではrelabelが効かない**（figureのキャプション後置で実測）: `ctx.removeNode(label)` → `ctx.insertAfter(last, label)` とすると移動自体は成功するが、その前後どちらで `ctx.setProperty(label, ...)` を呼んでも**relabelがdropされて`<p>`のまま**出力される → `structuredClone(label)` したコピーに `data`（`hName`等）を**リテラル時点で直付け**して挿入する
6. **`containerDirective`型のままだと内部の脚注`[^n]`が変換されない**（実測）: ネイティブ変換器の脚注参照収集パスが`containerDirective`の中を走査しないため、mdast上は`footnoteReference`ノードになっていても、hast変換で`[^1]`の原文テキストに落ち、脚注セクションからも該当定義が消える → visitorで同じchildrenを持つ**`blockquote`ノードへ`replaceNode`**し、`data.hName`で`details`/`aside`/`figure`に相殺する（blockquoteは収集パスの走査対象なので内部の脚注が通常どおり番号付け・変換される）。childrenには元のハンドルをそのまま渡してよく、子への`setProperty`（summary化等）は置換後も適用される（実測）。ネスト（外側replaceNode後の内側directiveの単独visit）も正常に動作する（実測）
