...
- type は `feat` / `fix` / `docs` / `test` / `refactor` / `chore` / `content` を使う。scope は任意。`content` は `content/` 配下のコンテンツ（辞書・記事・本）の追加・更新に使う（例: `content: 辞書「所有権」を追加`）
...
実装とレビューは専用のスラッシュコマンド（`.claude/commands/`）で定型化している。`/impl` → `/task-review` →（指摘があれば）`/task-fix` → `/task-review` の順に回す。
...
- **`/task-fix <タスクID> [対応範囲]`**: レビュー結果ファイルの最新レビューブロックから指摘を読み、対応方針の承認を得た上で修正〜検証〜コミットを行う。対応範囲は `要修正`（既定）/ `推奨` / `全部`。対応結果は同じレビュー結果ファイルの `## 対応記録` に追記する。修正コミットにも `Task: <タスクID>` トレーラーを付けるため、そのまま `/task-review` で再レビューできる

レビュー結果ファイルは実装セッションとレビューセッションの受け渡し口を兼ねる。別セッションへ引き継ぐ場合は、指摘内容をメッセージ本文に書き写さず `docs/archive/review/<タスクID>.md` のパスを伝える（SSoT）。
