...
レビュー対応（[レビュー結果](../archive/review/T1-4.md)・**承認**／要修正なし）: 推奨R-1（AC-9テストがR-15の「リンク元ファイル」含有を未検証）に対応し、`tests/plugins/wikilink.test.ts` にリンク元ファイル名のアサーションを追加。軽微M-1〜M-3はレビューで「対応不要」判定のため見送り。
