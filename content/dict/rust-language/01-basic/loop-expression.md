...
ループの前に`'名前:`の[[loop-label]]を付けると、ネストした内側から`break '名前`・`continue '名前`で外側のループを直接操作できます。ラベルは`loop`だけでなく`while`・`for`にも付けられます[^1]。なお、ループ以外でもラベル付きブロック式`'名前: { ... }`なら`break '名前 値;`で値を返せます。
