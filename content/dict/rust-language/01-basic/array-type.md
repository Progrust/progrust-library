...
要素には`array[index]`という添字アクセスで取得します。添字は`usize`型でなければならず、実行時に境界チェックされるため、範囲外を指定すると即座にパニックします。パニックさせずに範囲外を検出したい場合は、`Option`を返す`get`メソッドを使います。全要素を順に処理したいときは`for`式で反復できます。
