...
真偽値を表すプリミティブ型`bool`です。取りうる値は`true`と`false`の2つのみで、メモリ上は1バイト（8ビット）で表現されます。[[integer-type]]や[[floating-point-type]]と同様に`Copy`を実装しており、`if`式・`while`式の条件式や、論理演算子（`&&`・`||`・`!`）のオペランドとして使われます。
