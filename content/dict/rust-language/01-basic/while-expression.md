...
条件の代わりにパターンを書く`while let`は、パターンがマッチする間だけ繰り返します[^1]。「値を取り出せる間ずっと処理する」という定番の書き方で、[[vec]]の`pop`のように[[option]]を返すメソッドと相性が良い構文です。
...
fn main() {
    ...
    let mut total = 0;
    while let Some(price) = stock.pop() && total + price <= 1000 {
