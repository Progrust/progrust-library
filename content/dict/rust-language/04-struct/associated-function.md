...
関連関数（associated function）は、implブロック内で特定の型に紐づけて定義する関数です。<!-- TODO: [[impl-block]] 作成後にリンク --> 第一引数に`self`を取るものは特に「[[method]]」と呼ばれます。一方、`self`を取らない関連関数は`型名::関数名()`の形で呼び出し、インスタンスがまだ無い状態でも使える点がメソッドと異なります。
