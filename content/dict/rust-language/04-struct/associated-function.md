...
関連関数（associated function）は、[[impl-block]]内で特定の型に紐づけて定義する関数です。第一引数に`self`を取るものは特に「[[method]]」と呼ばれます。一方、`self`を取らない関連関数は`型名::関数名()`の形で呼び出し、インスタンスがまだ無い状態でも使える点がメソッドと異なります。
...
`値.method()`というメソッド呼び出し構文でも、実体は`self`を取る関連関数の呼び出しです。コンパイラが受け手の型に応じて[[reference]]の付与・[[dereference]]を自動で行う（[[auto-deref]]）ため、`User::method(&user)`のように通常の関連関数と同じ形式でも呼び出せます（実用上はメソッド構文を使います）。
:::
