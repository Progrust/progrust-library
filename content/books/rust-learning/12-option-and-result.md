...
第11章で、列挙型と`match`式を身につけました。この章では、その2つが実際にどう使われているのか——標準ライブラリの`Option`型と`Result`型——を13問で扱います。
...
:::message{tip}
これで第12章は終わりです。ここまでで、Rustの言語機能を一通り扱いました。次の第13章からは**プロジェクト構成編**として、書けるようになったコードをどう整理し、どう分割し、どこまでを外に見せるかを扱います。まずは`mod`によるモジュールと、`pub`による公開範囲の指定からです。
:::
::::
